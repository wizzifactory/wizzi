/*
    ...
    primary source IttfDocument: C:\My\wizzi\wizzi\packages\wizzi\.wizzi\ittf\examples\plugins\sample\index.js.ittf
*/
'use strict';
/**
...
executeGenerateSchema('sample', function(err, notUsed) {
    if (err) {
        console.log('executeGenerateSchema.err', err);
    }
    else {
        executeGenerateModules([
            ...
            if (err) {
                console.log('generate.index.err', err);
            }
            else {
                executeCopy('examples/ittf/first.sample.ittf');
                console.log('done');
            }
        });
    }
...
function executeGenerateSchema(schema, callback) {
    var ittfPath = path.join(__dirname, '_wizzi', 'ittf', 'lib', 'wizzi', 'schemas', schema + '.wfschema.ittf');
...
function executeGenerateModules(modules, callback) {
    async.mapSeries(modules, (module, callback) => {
        var ittfDocumentUri = path.join(__dirname, '_wizzi', 'ittf', module + '.js.ittf');
        var outputPath = path.join(__dirname, 'dist', module + '.js');
        loadModelAndGenerateArtifact(ittfDocumentUri, {}, 'js/module', function(err, artifactText) {
            if (err) {
                return callback(err);
            }
            file.write(outputPath, artifactText);
...
function executeCopy(relPath) {
    var inputPath = path.join(__dirname, '_wizzi', 'ittf', relPath);
    var outputPath = path.join(__dirname, 'dist', relPath);
    file.write(outputPath, file.read(inputPath));
}
