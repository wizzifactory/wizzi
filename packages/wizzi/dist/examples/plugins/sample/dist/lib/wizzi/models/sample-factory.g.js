...
'use strict';
/**
     sample WizziModelFactory
*/
var path = require('path');
var util = require('util');
var _ = require('lodash');
var stringify = require('json-stringify-safe');
var samplemTreePreProcessor = require('./sample-mtree-preprocessor.g');
var sampleschema = require('./sample-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var samplemodel = new samplemodelType('EmptyIttfDocument');
        }
        else {
            // Get the model type of the root node of the ittf model.
            var rootNode = mTree.nodes[0];
            var samplemodelType = sampleschema[rootNode.n];
            if (!samplemodelType) {
                var maptag = sampleschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    samplemodelType = sampleschema[maptag];
                }
                if (!samplemodelType) {
                    var error = new errors.WizziModelLoadError('In sample Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: sample', ittfDocumentUri);
                    return callback(error);
                }
            }
            // Load the WizziModel from the root node of the mTree
            samplemodel = new samplemodelType(rootNode.v);
            samplemodel.loadHistory = mTree.loadHistory;
            samplemodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                samplemodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn sample Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new sampleschema.sampleContext();
        samplemodel.wzInitialize(ctx);
        samplemodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In sample Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        // _ log.info('Initialized wmt model ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && samplemodel.toJson && file.isFilePath(ittfDocumentUri)) {
            // dump for debug
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(samplemodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        samplemodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && samplemodel.toJson && file.isFilePath(ittfDocumentUri)) {
                // dump for debug
            ...
            callback(null, samplemodel);
        });
    }
    if (options.loadFromMTree) {
        /**
             Load a WizziModel of type 'sample' from an mTree
        */
        ...
        /**
             Load a WizziModel of type 'sample' from an IttfDocument uri
                params
        ...
        */
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema sample from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
                    ...
                    mTree = samplemTreePreProcessor(mTree, loadContext);
