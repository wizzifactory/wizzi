/*
    ...
    utc time: Tue, 11 Jun 2019 14:07:16 GMT
*/
'use strict';
/**
     vtt WizziModelFactory
*/
var path = require('path');
var util = require('util');
var _ = require('lodash');
var stringify = require('json-stringify-safe');
var vttschema = require('./vtt-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var vttmodel = new vttmodelType('EmptyIttfDocument');
        }
        else {
            // Get the model type of the root node of the ittf model.
            var rootNode = mTree.nodes[0];
            var vttmodelType = vttschema[rootNode.n];
            if (!vttmodelType) {
                var maptag = vttschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    vttmodelType = vttschema[maptag];
                }
                if (!vttmodelType) {
                    var error = new errors.WizziModelLoadError('In vtt Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: vtt', ittfDocumentUri);
                    return callback(error);
                }
            }
            // Load the WizziModel from the root node of the mTree
            vttmodel = new vttmodelType(rootNode.v);
            vttmodel.loadHistory = mTree.loadHistory;
            vttmodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                vttmodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn vtt Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new vttschema.vttContext();
        vttmodel.wzInitialize(ctx);
        vttmodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In vtt Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        // _ log.info('Initialized wmt model ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && vttmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            // dump for debug
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(vttmodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        vttmodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && vttmodel.toJson && file.isFilePath(ittfDocumentUri)) {
                // dump for debug
            ...
            callback(null, vttmodel);
        });
    }
    if (options.loadFromMTree) {
        /**
             Load a WizziModel of type 'vtt' from an mTree
        */
        ...
        /**
             Load a WizziModel of type 'vtt' from an IttfDocument uri
                params
        ...
        */
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema vtt from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
