/*
    ...
    utc time: Fri, 14 Jun 2019 13:46:05 GMT
*/
'use strict';
/**
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var cssmodel = new cssmodelType('EmptyIttfDocument');
        }
        else {
            ...
            cssmodel.loadHistory = mTree.loadHistory;
            cssmodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                cssmodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn css Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && cssmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            // dump for debug
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(cssmodel.toJson(), null, 2));
        }
        ...
        cssmodel.wzInitializeAsync(ctx, function(err, result) {
            ...
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && cssmodel.toJson && file.isFilePath(ittfDocumentUri)) {
                // dump for debug
    ...
    if (options.loadFromMTree) {
        /**
             Load a WizziModel of type 'css' from an mTree
        */
