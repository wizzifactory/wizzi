/*
    ...
    utc time: Fri, 14 Jun 2019 13:46:06 GMT
*/
'use strict';
/**
...
var htmlmTreePreProcessor = require('./html-mtree-preprocessor.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            ...
            htmlmodel.loadHistory = mTree.loadHistory;
            htmlmodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                htmlmodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn html Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && htmlmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            // dump for debug
        ...
        htmlmodel.wzInitializeAsync(ctx, function(err, result) {
            ...
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && htmlmodel.toJson && file.isFilePath(ittfDocumentUri)) {
                // dump for debug
    ...
    if (options.loadFromMTree) {
        /**
             Load a WizziModel of type 'html' from an mTree
        */
        ...
        */
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    mTree = htmlmTreePreProcessor(mTree, loadContext);
