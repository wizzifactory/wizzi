/*
    ...
    primary source IttfDocument: C:\My\wizzi\wizzi-mono\packages\wizzi-scripts\.wizzi\ittf\examples\webpack\index.js.ittf
*/
'use strict';
...
var options1 = {
    entry: path.join(codecampPath, 'react', 'src'), 
    output: {
        path: path.join(codecampPath, 'react', 'dist'), 
        filename: 'bundle.js'
    }, 
    useReact: true
};
webpack(function(err, result) {
    if (err) {
        console.log('err', err);
    ...
    /**
        webpackToEventStream(function(err, result) {
            if (err) {
                console.log('err', err);
                throw new Error(err.message);
            }
            console.log('webpackToEventStream.result', result);
        })*/
});
function webpack(callback) {
    manager.webpackExec(options1, callback);
}
function webpackToEventStream(callback) {
    var res = new MockResponse();
    manager.webpackExecToEventStream(options1, res, function(err, result) {
        if (err) {
            console.log('err', err);
            throw new Error(err.message);
        }
