/*
    ...
    primary source IttfDocument: C:\My\wizzi\wizzi-mono\packages\wizzi-scripts\.wizzi\ittf\examples\powershell\index.js.ittf
*/
'use strict';
...
var psOptions1 = {
    scriptPath: path.join(__dirname, 'scripts', 'sample1.ps1'), 
    params: [
        {
            name: 'str', 
...
ps(function(err, result) {
    ...
    psToEventStream(function(err, result) {
        if (err) {
            console.log('err', err);
            throw new Error(err.message);
        }
        console.log('psToEventStream.result', result);
    });
});
function ps(callback) {
    manager.powershellExec(psOptions1, callback);
}
function psToEventStream(callback) {
    var res = new MockResponse();
    manager.powershellExecToEventStream(psOptions1, res, function(err, result) {
        if (err) {
            console.log('err', err);
            throw new Error(err.message);
        }
