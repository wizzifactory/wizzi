...
var nodeOptions1 = {
    scriptPath: path.join(__dirname, 'scripts', 'node1.js'), 
    args: [
        'stefi'
    ]
};
nodeJs(function(err, result) {
    if (err) {
        console.log('err', err);
        throw new Error(err.message);
    }
    console.log('nodeJs.result', result);
    nodeJsToEventStream(function(err, result) {
        if (err) {
            console.log('err', err);
            throw new Error(err.message);
        }
        console.log('nodeJsToEventStream.result', result);
    });
});
function nodeJs(callback) {
    manager.nodeJsExec(nodeOptions1, callback);
}
function nodeJsToEventStream(callback) {
    var res = new MockResponse();
    manager.nodeJsExecToEventStream(nodeOptions1, res, function(err, result) {
        if (err) {
            console.log('err', err);
            throw new Error(err.message);
        }
