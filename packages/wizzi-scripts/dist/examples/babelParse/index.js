/*
    ...
    primary source IttfDocument: C:\My\wizzi\wizzi-mono\packages\wizzi-scripts\.wizzi\ittf\examples\babelParse\index.js.ittf
*/
'use strict';
...
babel(function(err, result) {
    ...
    file.write(path.join(codecampPath, 'react', 'dist', fileBasename + '.ast'), JSON.stringify(result, null, 2));
    /**
        babel(function(err, result) {
            if (err) {
                console.log('err', err);
...
function babel(callback) {
    manager.babelParseExec(options1, callback);
}
function babelParseToEventStream(callback) {
    var res = new MockResponse();
    manager.babelParseToEventStream(options1, res, function(err, result) {
        if (err) {
            console.log('err', err);
            throw new Error(err.message);
        }
