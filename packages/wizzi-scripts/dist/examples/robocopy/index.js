/*
    ...
    primary source IttfDocument: C:\My\wizzi\wizzi-mono\packages\wizzi-scripts\.wizzi\ittf\examples\robocopy\index.js.ittf
*/
'use strict';
...
var rcOptions1 = {
    source: path.join(__dirname, 'data', 'from'), 
    destination: [
        path.join(__dirname, 'data', 'to1'), 
        path.join(__dirname, 'data', 'to2')
    ], 
    serial: false, 
    files: [
...
robo(function(err, result) {
    if (err) {
        console.log('err', err);
        throw new Error(err.message);
    }
    console.log('robo.result', result);
    roboToEventStream(function(err, result) {
        if (err) {
            console.log('err', err);
            throw new Error(err.message);
        }
        console.log('roboToEventStream.result', result);
    });
});
function robo(callback) {
    manager.robocopyExec(rcOptions1, callback);
}
function roboToEventStream(callback) {
    var res = new MockResponse();
    manager.robocopyExecToEventStream(rcOptions1, res, function(err, result) {
        if (err) {
            console.log('err', err);
            throw new Error(err.message);
        }
