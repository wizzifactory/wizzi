/*
    ...
    primary source IttfDocument: C:\My\wizzi\wizzi-mono\packages\wizzi-scripts\.wizzi\ittf\examples\flow\index.js.ittf
*/
'use strict';
...
flow(function(err, result) {
    if (err) {
        console.log('err', err);
        throw new Error(err.message);
    }
    console.log('flow.result', result);
    flowToEventStream(function(err, result) {
        if (err) {
            console.log('err', err);
            throw new Error(err.message);
        }
        console.log('flowToEventStream.result', result);
    });
});
function flow(callback) {
    manager.flowExec(options1, callback);
}
function flowToEventStream(callback) {
    var res = new MockResponse();
    manager.flowExecToEventStream(options1, res, function(err, result) {
        if (err) {
            console.log('err', err);
            throw new Error(err.message);
        }
