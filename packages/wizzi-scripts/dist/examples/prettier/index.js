/*
    ...
    primary source IttfDocument: C:\My\wizzi\wizzi-mono\packages\wizzi-scripts\.wizzi\ittf\examples\prettier\index.js.ittf
*/
'use strict';
...
prettier(function(err, result) {
    if (err) {
        console.log('err', err);
        throw new Error(err.message);
    }
    console.log('prettier.result', result);
    file.write(path.join(codecampPath, 'react', 'dist', fileBasename + '.prettier.js'), result);
    /**
        prettier(function(err, result) {
            if (err) {
                console.log('err', err);
            ...
            console.log('prettierToEventStream.result', result);
        })*/
});
function prettier(callback) {
    manager.prettierExec(options1, callback);
}
function prettierToEventStream(callback) {
    var res = new MockResponse();
    manager.prettierToEventStream(options1, res, function(err, result) {
        if (err) {
            console.log('err', err);
            throw new Error(err.message);
        }
