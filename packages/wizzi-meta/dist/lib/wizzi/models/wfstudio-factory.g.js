/*
    ...
    utc time: Wed, 24 Apr 2019 06:08:04 GMT
*/
'use strict';
/**
     wfstudio WizziModelFactory
*/
var path = require('path');
var util = require('util');
var _ = require('lodash');
var stringify = require('json-stringify-safe');
var wfstudioschema = require('./wfstudio-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var wfstudiomodel = new wfstudiomodelType('EmptyIttfDocument');
        }
        else {
            // Get the model type of the root node of the ittf model.
            var rootNode = mTree.nodes[0];
            var wfstudiomodelType = wfstudioschema[rootNode.n];
            if (!wfstudiomodelType) {
                var maptag = wfstudioschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    wfstudiomodelType = wfstudioschema[maptag];
                }
                if (!wfstudiomodelType) {
                    var error = new errors.WizziModelLoadError('In wfstudio Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: wfstudio', ittfDocumentUri);
                    return callback(error);
                }
            }
            // Load the WizziModel from the root node of the mTree
            wfstudiomodel = new wfstudiomodelType(rootNode.v);
            wfstudiomodel.loadHistory = mTree.loadHistory;
            wfstudiomodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                wfstudiomodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn wfstudio Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new wfstudioschema.wfstudioContext();
        wfstudiomodel.wzInitialize(ctx);
        wfstudiomodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In wfstudio Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        // _ log.info('Initialized wmt model ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && wfstudiomodel.toJson && file.isFilePath(ittfDocumentUri)) {
            // dump for debug
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(wfstudiomodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        wfstudiomodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && wfstudiomodel.toJson && file.isFilePath(ittfDocumentUri)) {
                // dump for debug
            ...
            callback(null, wfstudiomodel);
        });
    }
    if (options.loadFromMTree) {
        /**
             Load a WizziModel of type 'wfstudio' from an mTree
        */
        ...
        /**
             Load a WizziModel of type 'wfstudio' from an IttfDocument uri
                params
        ...
        */
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema wfstudio from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
