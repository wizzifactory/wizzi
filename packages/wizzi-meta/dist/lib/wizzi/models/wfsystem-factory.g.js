...
'use strict';
/**
     wfsystem WizziModelFactory
*/
var path = require('path');
var util = require('util');
var _ = require('lodash');
var stringify = require('json-stringify-safe');
var wfsystemschema = require('./wfsystem-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            ...
            var xpackagemodelType = wfsystemschema[rootNode.n];
            if (!xpackagemodelType) {
                var maptag = wfsystemschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    xpackagemodelType = wfsystemschema[maptag];
                }
                if (!xpackagemodelType) {
                    var error = new errors.WizziModelLoadError('In wfsystem Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: wfsystem', ittfDocumentUri);
                    return callback(error);
                }
            }
            // Load the WizziModel from the root node of the mTree
            xpackagemodel = new xpackagemodelType(rootNode.v);
            xpackagemodel.loadHistory = mTree.loadHistory;
            xpackagemodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                xpackagemodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn wfsystem Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new wfsystemschema.wfsystemContext();
        ...
        xpackagemodel.wzInitializeAsync(ctx, function(err, result) {
            ...
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && xpackagemodel.toJson && file.isFilePath(ittfDocumentUri)) {
                // dump for debug
    ...
    if (options.loadFromMTree) {
        /**
             Load a WizziModel of type 'wfsystem' from an mTree
        */
        ...
        /**
             Load a WizziModel of type 'wfsystem' from an IttfDocument uri
                params
        ...
        */
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema wfsystem from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
