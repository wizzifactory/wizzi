...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            ...
            wfpackagemodel.loadHistory = mTree.loadHistory;
            wfpackagemodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                wfpackagemodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn wfpackage Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && wfpackagemodel.toJson && file.isFilePath(ittfDocumentUri)) {
            // dump for debug
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(wfpackagemodel.toJson(), null, 2));
        }
        ...
        wfpackagemodel.wzInitializeAsync(ctx, function(err, result) {
            ...
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && wfpackagemodel.toJson && file.isFilePath(ittfDocumentUri)) {
                // dump for debug
    ...
    if (options.loadFromMTree) {
        /**
             Load a WizziModel of type 'wfpackage' from an mTree
        */
