...
var wfstudioBase = (function () {
    ...
    wfstudioBase.prototype.wzSourceErrorLines = function(node, message) {
...
var wfstudio = (function (wfstudioitem) {
    ...
    wfstudio.prototype.toJsonChildren = function(result) {
        ...
        var items, item;
        items = [];
        var i, i_items=this.projects, i_len=this.projects.length, item;
        for (i=0; i<i_len; i++) {
    ...
    wfstudio.prototype.wzInitialize = function(ctx) {
        ...
        var i, i_items=this.packages, i_len=this.packages.length, p;
        for (i=0; i<i_len; i++) {
            p = this.packages[i];
            packageEx = {
                name: p.wzName, 
            ...
            var j, j_items=p.folders, j_len=p.folders.length, f;
            for (j=0; j<j_len; j++) {
                f = p.folders[j];
                folder = {
                    name: f.wzName, 
                ...
                var k, k_items=f.aspects, k_len=f.aspects.length, a;
                for (k=0; k<k_len; k++) {
                    a = f.aspects[k];
            ...
            var schema, artifact;
            var j, j_items=p.schemas, j_len=p.schemas.length, s;
            for (j=0; j<j_len; j++) {
                s = p.schemas[j];
                schema = {
                    package: p.wzName, 
                ...
                var k, k_items=s.artifacts, k_len=s.artifacts.length, a;
                for (k=0; k<k_len; k++) {
                    a = s.artifacts[k];
                    artifact = {
        ...
        var i, i_items=this.projects, i_len=this.projects.length, p;
        for (i=0; i<i_len; i++) {
            p = this.projects[i];
...
var folder = (function (wfstudioitem) {
    ...
    folder.prototype.toJsonChildren = function(result) {
        ...
        var items, item;
        items = [];
        var i, i_items=this.aspects, i_len=this.aspects.length, item;
        for (i=0; i<i_len; i++) {
            item = this.aspects[i];
            items.push(item.toJson());
        }
...
var xpackage = (function (wfstudioitem) {
    ...
    xpackage.KIND_KERNEL = "kernel";xpackage.KIND_PLUGIN = "plugin";xpackage.KIND_APP = "app";xpackage.KIND_DEMO = "demo";xpackage.prototype.addFolder = function(name, sourceLineInfo) {
        var retval = new _md.folder(name, sourceLineInfo);
        retval.wzParent = this;
        this.folders.push(retval);
        return retval;
    ...
    xpackage.prototype.wzInitialize = function(ctx) {
        ...
        for (i=0; i<i_len; i++) {
            f = this.folders[i];
            var j, j_items=f.aspects, j_len=f.aspects.length, a;
            for (j=0; j<j_len; j++) {
                a = f.aspects[j];
...
var schema = (function (wfstudioitem) {
    ...
    schema.prototype.toJsonChildren = function(result) {
        ...
        items = [];
        var i, i_items=this.transforms, i_len=this.transforms.length, item;
        for (i=0; i<i_len; i++) {
    ...
    schema.prototype.wzInitialize = function(ctx) {
        ...
        var i, i_items=this.artifacts, i_len=this.artifacts.length, a;
        for (i=0; i<i_len; i++) {
            a = this.artifacts[i];
            if (a.isDefault) {
        ...
        var i, i_items=this.transforms, i_len=this.transforms.length, t;
        for (i=0; i<i_len; i++) {
            t = this.transforms[i];
            if (t.isDefault) {
