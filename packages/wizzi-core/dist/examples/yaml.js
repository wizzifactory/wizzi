/*
    ...
    primary source IttfDocument: C:\My\wizzi\wizzi\packages\wizzi-core\.wizzi\ittf\examples\yaml.js.ittf
*/
'use strict';
...
executeLoadFolderModels('yamls', 'yaml', {}, function(err, notUsed) {
    if (err) {
        return callback(err);
    }
});
...
function executeLoadModel(name, folder, schema, context, callback) {
    ...
    var artifactOutput = path.join(__dirname, 'outputs', folder, name + '.artifact.yaml');
    ...
    loadWizziModel(ittfSource, context || {}, function(err, jsWizziModel) {
        ...
        loadModelAndGenerateArtifact(ittfSource, context || {}, "yaml/document", function(err, artifactText) {
            if (err) {
                console.log('err', err);
                throw new Error(err.message);
            }
