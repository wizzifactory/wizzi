...
'use strict';
/**
     temp WizziModelFactory
*/
var path = require('path');
var util = require('util');
var _ = require('lodash');
var stringify = require('json-stringify-safe');
var tempschema = require('./temp-model.g');
...
md.createLoadModel = function(wizziObject) {
    ...
    function loadModelFromMTree(mTree, ittfDocumentUri, wizziModelRequest, options, callback) {
        ...
        if (mTree.nodes.length == 0) {
            var tempmodel = new tempmodelType('EmptyIttfDocument');
        }
        else {
            // Get the model type of the root node of the ittf model.
            var rootNode = mTree.nodes[0];
            var tempmodelType = tempschema[rootNode.n];
            if (!tempmodelType) {
                var maptag = tempschema.__tagElementMapping[rootNode.n];
                if (typeof maptag === 'string') {
                    tempmodelType = tempschema[maptag];
                }
                if (!tempmodelType) {
                    var error = new errors.WizziModelLoadError('In temp Factory. Cannot map root node: ' + rootNode.n + ', to any entity of schema: temp', ittfDocumentUri);
                    return callback(error);
                }
            }
            // Load the WizziModel from the root node of the mTree
            tempmodel = new tempmodelType(rootNode.v);
            tempmodel.loadHistory = mTree.loadHistory;
            tempmodel.wzFactory = options.wizziFactory;
            try {
                // this is a sync call
                tempmodel.loadFromNode(rootNode);
            } 
            catch (ex) {
                var error = new errors.WizziModelLoadError(ex.message + '\nIn temp Factory, calling loadFromNode.', ittfDocumentUri, ex);
        ...
        var ctx = new tempschema.tempContext();
        tempmodel.wzInitialize(ctx);
        tempmodel.wzVerify(ctx);
        if (ctx.schemaIsValid() === false) {
            var errorsMessage = ctx.validationErrors.join('\n');
            var error = new errors.WizziModelLoadError('In temp Factory.\nWizziModel has validation errors: \n' + errorsMessage, ittfDocumentUri);
            callback(error);
        }
        // TODO implement a stats object inside the wizziModelRequest object
        // _ log.info('Initialized wmt model ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
        if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModel) && tempmodel.toJson && file.isFilePath(ittfDocumentUri)) {
            // dump for debug
            var mTreeDump = path.join(path.dirname(ittfDocumentUri), '_debug', path.basename(ittfDocumentUri) + '.dump.json');
            file.write(mTreeDump, stringify(tempmodel.toJson(), null, 2));
        }
        // TODO Generate this wzInitializeAsync call only if wizziModelRequested by the wizzischema
        tempmodel.wzInitializeAsync(ctx, function(err, result) {
            if (err) {
                return callback(err, null);
            }
            if ((wizziModelRequest.dumpAll || wizziModelRequest.dumpModelAfterInitializeAsync) && tempmodel.toJson && file.isFilePath(ittfDocumentUri)) {
                // dump for debug
            ...
            callback(null, tempmodel);
        });
    }
    if (options.loadFromMTree) {
        /**
             Load a WizziModel of type 'temp' from an mTree
        */
        ...
        /**
             Load a WizziModel of type 'temp' from an IttfDocument uri
                params
        ...
        */
        return function loadModel(ittfDocumentUri, loadContext, callback) {
                ...
                loadMTree(ittfDocumentUri, loadContext, function(err, mTree) {
                    ...
                    // TODO implement a stats object inside the wizziModelRequest object
                    // _ log.info('Loaded Wizzi model instance of schema temp from Ittf document ' + ittfDocumentUri + ' in ' + (Date.now() - start) + ' ms')
